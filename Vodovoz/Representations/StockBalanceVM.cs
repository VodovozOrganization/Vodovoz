...
{
	public class StockBalanceVM : RepresentationModelEntityBase<Nomenclature, StockBalanceVMNode>
	{
		public StockBalanceFilter Filter {
			get => RepresentationFilter as StockBalanceFilter;
	...
	public class StockBalanceVMNode
	{
		...
		public string NomenclatureName {
			get => string.Format("{0}{1}", nomenclatureName, NomenclatureIsArchive ? " (АРХИВ)" : "");
			set => nomenclatureName = value;
		}

		public string CountText => String.Format("{0:" + String.Format("F{0}", UnitDigits) + "} {1}", Amount, UnitName);

		public string MinCountText => string.Format("{0:" + String.Format("F{0}", UnitDigits) + "} {1}", NomenclatureMinCount, UnitName);

		public string DiffCountText => string.Format("{0:" + String.Format("F{0}", UnitDigits) + "} {1}", DiffCount, UnitName);


		public decimal Amount => Append - Removed;

		public decimal NomenclatureMinCount { get; set; }

		public decimal DiffCount => Amount - NomenclatureMinCount;

		public string RowColor => Amount < 0 ? "red" : "black";
	}
}