...
namespace Vodovoz.Additions.Logistic.RouteOptimization
{
	...
	public class CallbackTime : NodeEvaluator2
	{
		...
		public override long Run(int first_index, int second_index)
		{
			...
			if(second_index == 0)
				travelTime = distanceCalculator.TimeToBaseSec(Nodes[first_index - 1].Order.DeliveryPoint, Nodes[first_index - 1].ShippingBase, Nodes[first_index - 1].Order.DeliveryDate);
			else if(first_index == 0)
				travelTime = distanceCalculator.TimeFromBaseSec(Nodes[second_index - 1].ShippingBase, Nodes[second_index - 1].Order.DeliveryPoint, Nodes[second_index - 1].Order.DeliveryDate);
			else
				travelTime = distanceCalculator.TimeSec(Nodes[first_index - 1].Order.DeliveryPoint, Nodes[second_index - 1].Order.DeliveryPoint, Nodes[first_index - 1].Order.DeliveryDate, Nodes[second_index - 1].Order.DeliveryDate);
