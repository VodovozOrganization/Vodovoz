...
namespace Vodovoz.ExportTo1c
{
	...
	<ДатаВремяСоздания>2020-12-19T00:58:18</ДатаВремяСоздания>
...
КонецЕсли;</ПоследовательностьПолейПоиска>
		...
		<Правило>
			...
			<ПослеЗагрузки>Объект.ВалютаВзаиморасчетов = Константы.ВалютаРегламентированногоУчета.Получить();</ПослеЗагрузки>
			<ПоследовательностьПолейПоиска>НоваяЗапись = РегистрыСведений.ОсновныеДоговорыКонтрагента.СоздатьМенеджерЗаписи();
...
ПрекратитьПоиск = Истина;</ПоследовательностьПолейПоиска>
		...
		<Группа>
			...
			<Правило>
				...
				<Порядок>50</Порядок>
				<ПослеЗагрузки>
					//Закомментировано, потому что эти заказы без сумммы - заказы со скидкой
					//Для Каждого ТекСтрока Из Объект.Товары Цикл    	
					//	Если НЕ ЗначениеЗаполнено(ТекСтрока.Сумма) Тогда  		
