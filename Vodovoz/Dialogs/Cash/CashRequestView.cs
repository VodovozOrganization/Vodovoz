...
namespace Vodovoz.Dialogs.Cash
{
	...
	public partial class CashRequestView : TabViewBase<CashRequestViewModel>
	{
		...
		private void ConfigureDlg()
		{
			...
			//Причина отмены
			yentryCancelReason.Binding.AddBinding(ViewModel.Entity, e => e.CancelReason, (widget) => widget.Buffer.Text)
				.InitializeFromSource();
			yentryCancelReason.WrapMode = WrapMode.Word;

			//Причина отправки на пересогласование
			yentryReasonForSendToReapproval.Binding
				.AddBinding(ViewModel.Entity, e => e.ReasonForSendToReappropriate, (widget) => widget.Buffer.Text).InitializeFromSource();
			...
			ybtnReturnForRenegotiation.Binding.AddBinding(ViewModel, vm => vm.CanReturnToRenegotiation, w => w.Visible)
				.InitializeFromSource();
			...
			ycheckPossibilityNotToReconcilePayments.Binding
				.AddBinding(ViewModel.Entity, e => e.PossibilityNotToReconcilePayments, w => w.Active).InitializeFromSource();
			ycheckPossibilityNotToReconcilePayments.Binding
				.AddBinding(ViewModel, vm => vm.CanConfirmPossibilityNotToReconcilePayments, w => w.Visible).InitializeFromSource();
			...
			ExpenseCategoryEntityviewmodelentry.Binding.AddBinding(ViewModel, vm => vm.ExpenseCategorySensitive, w => w.Sensitive)
				.InitializeFromSource();
