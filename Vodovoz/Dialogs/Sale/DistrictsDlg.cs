...
{
	public partial class DistrictsDlg : QS.Dialog.Gtk.TdiTabBase
	{
		...
		GenericObservableList<District> observableRestrictedDistricts;
		...
		IEnumerable<WageDistrict> wageDistricts;

		public DistrictsDlg(IUserPermissionRepository userPermission)
		{
			this.Build();
			Configure();
		...
		void Configure()
		{
			...
			ytreeDistricts.ColumnsConfig = FluentColumnsConfig<District>.Create()
			...
			ytreeSchedules.ColumnsConfig = FluentColumnsConfig<DeliveryScheduleRestriction>.Create()
				.AddColumn("График").HeaderAlignment(0.5f).AddTextRenderer(x => x.DeliverySchedule.Name)
				.Finish();
		...
		{
			currentDistrict = ytreeDistricts.GetSelectedObject() as District;
		...
		protected void OnBtnEditDistrictClicked(object sender, EventArgs e)
		{
			...
			TabParent.OpenTab(
				DialogHelper.GenerateDialogHashName<District>(currentDistrict.Id),
				() => new CommonDistrictRuleItemsDlg(currentDistrict)
			);
		}
		...
		protected void OnButtonDeleteDistrictClicked(object sender, EventArgs e)
		{
			if(currentDistrict.Id != 0 && QSMain.User.Admin)
				OrmMain.DeleteObject(typeof(District), currentDistrict.Id);
			else if(currentDistrict.Id == 0) {
				var mapPolygon = bordersOverlay.Polygons.FirstOrDefault(p => (p.Tag as District) == currentDistrict);
				if(mapPolygon != null)
		...
		void SelectSchedules_ObjectSelected(object sender, OrmReferenceObjectSectedEventArgs e)
		{
			...
			if(ytreeSchedules.ItemsDataSource is GenericObservableList<DeliveryScheduleRestriction> scheduleRestrictions){
				foreach (var item in e.Subjects) {
					if (item is DeliverySchedule schedule &&
					    scheduleRestrictions.All(x => x.DeliverySchedule.Id != schedule.Id))
						scheduleRestrictions.Add(new DeliveryScheduleRestriction {
							District = currentDistrict, WeekDay = selectedWeekDayName.Value, DeliverySchedule = schedule
						});
				}
			}
		}
		...
		{
			var scheduleList = ytreeSchedules.ItemsDataSource as GenericObservableList<DeliveryScheduleRestriction>;
			if(ytreeSchedules.GetSelectedObject() is DeliveryScheduleRestriction selectedObj)
				scheduleList?.Remove(selectedObj);
		}

		public virtual GenericObservableList<District> ObservableRestrictedDistricts {
			get {
				if(observableRestrictedDistricts == null) {
					observableRestrictedDistricts = new GenericObservableList<District>(GetAllDistricts());
				}
		...
		IList<District> GetAllDistricts()
		{
			var districts = uow.Session.QueryOver<District>()
				.List<District>().OrderBy(x => x.TariffZone.Name, new NaturalStringComparer()).ThenBy(x => x.DistrictName).ToList();
		...
		{
			foreach(District district in observableRestrictedDistricts) {
				var valid = new QSValidator<District>(district);
		...
		protected void OnBtnTodayClicked(object sender, EventArgs e)
		{
			...
			ytreeSchedules.ItemsDataSource = currentDistrict.ObservableTodayDeliveryScheduleRestrictions;
			ytreeSchedules.QueueDraw();
		}

		protected void OnBtnMondayClicked(object sender, EventArgs e)
		{
			selectedWeekDayName = WeekDayName.Monday;
			ytreeSchedules.ItemsDataSource = currentDistrict.ObservableMondayDeliveryScheduleRestrictions;
			ytreeSchedules.QueueDraw();
		}

		protected void OnBtnTuesdayClicked(object sender, EventArgs e)
		{
			selectedWeekDayName = WeekDayName.Tuesday;
			ytreeSchedules.ItemsDataSource = currentDistrict.ObservableTuesdayDeliveryScheduleRestrictions;
			ytreeSchedules.QueueDraw();
		}

		protected void OnBtnWednesdayClicked(object sender, EventArgs e)
		{
			selectedWeekDayName = WeekDayName.Wednesday;
			ytreeSchedules.ItemsDataSource = currentDistrict.ObservableWednesdayDeliveryScheduleRestrictions;
			ytreeSchedules.QueueDraw();
		}

		protected void OnBtnThursdayClicked(object sender, EventArgs e)
		{
			selectedWeekDayName = WeekDayName.Thursday;
			ytreeSchedules.ItemsDataSource = currentDistrict.ObservableThursdayDeliveryScheduleRestrictions;
			ytreeSchedules.QueueDraw();
		}

		protected void OnBtnFridayClicked(object sender, EventArgs e)
		{
			selectedWeekDayName = WeekDayName.Friday;
			ytreeSchedules.ItemsDataSource = currentDistrict.ObservableFridayDeliveryScheduleRestrictions;
			ytreeSchedules.QueueDraw();
		}

		protected void OnBtnSaturdayClicked(object sender, EventArgs e)
		{
			selectedWeekDayName = WeekDayName.Saturday;
			ytreeSchedules.ItemsDataSource = currentDistrict.ObservableSaturdayDeliveryScheduleRestrictions;
			ytreeSchedules.QueueDraw();
		}

		protected void OnBtnSundayClicked(object sender, EventArgs e)
		{
			selectedWeekDayName = WeekDayName.Sunday;
			ytreeSchedules.ItemsDataSource = currentDistrict.ObservableSundayDeliveryScheduleRestrictions;
			ytreeSchedules.QueueDraw();
		}
		...
		void SelectGeographicGroups_ObjectSelected(object sender, OrmReferenceObjectSectedEventArgs e)
		{
			if(yTreeGeographicGroups.ItemsDataSource is GenericObservableList<GeographicGroup> ggList)
				foreach(var item in e.Subjects) {
					if(item is GeographicGroup group && ggList.All(x => x.Id != @group.Id))
						ggList.Add(group);
				}
		}
