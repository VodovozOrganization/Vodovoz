...
namespace Vodovoz.Tools
{
	public class LoggedMySqlClientDriver : MySqlDataDriver
	{
		...
		private readonly DbType[] _typesWithQuotes = { DbType.Date, DbType.DateTime, DbType.String };
		...
		protected override void OnBeforePrepare(DbCommand command)
		{
			...
			{
				_logger.Error(ex, "Ошибка при формировании текста SQL запроса для логгера");
			}
		}
		...
		public string GetResultQueryText(DbCommand dbCommand)
		{
			...
			foreach(DbParameter parameter in dbCommand.Parameters)
			{
				...
				else if(parameter.DbType == DbType.Date || parameter.DbType == DbType.DateTime)
				{
				...
				parameterValue = _typesWithQuotes.Contains(parameter.DbType) ? "'" + parameterValue + "'" : parameterValue;
