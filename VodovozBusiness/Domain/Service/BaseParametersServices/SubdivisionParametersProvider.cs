...
namespace Vodovoz.Domain.Service.BaseParametersServices
{
	public class SubdivisionParametersProvider : ISubdivisionService
	{
		...
		public int GetOkkId()
		{
			if(!SingletonParametersProvider.Instance.ContainsParameter("номер_отдела_ОКК")) {
				throw new InvalidProgramException("В параметрах базы не настроен параметр : номер_отдела_ОКК");
			}
			return int.Parse(SingletonParametersProvider.Instance.GetParameterValue("номер_отдела_ОКК"));
		}

		public int GetSubdivisionIdForRLAccept()
		{
			if(!SingletonParametersProvider.Instance.ContainsParameter("accept_route_list_subdivision_restrict")) {
				throw new InvalidOperationException(String.Format("В базе не настроен параметр: accept_route_list_subdivision_restrict"));
			}
			return int.Parse(SingletonParametersProvider.Instance.GetParameterValue("accept_route_list_subdivision_restrict"));
		}

		public int GetParentVodovozSubdivisionId()
		{
			if(!SingletonParametersProvider.Instance.ContainsParameter("Id_Главного_подразделения_Веселый_Водовоз")) {
				throw new InvalidProgramException("В параметрах базы не настроен параметр : Id_Главного_подразделения_Веселый_Водовоз");
			}
			string value = SingletonParametersProvider.Instance.GetParameterValue("Id_Главного_подразделения_Веселый_Водовоз");
