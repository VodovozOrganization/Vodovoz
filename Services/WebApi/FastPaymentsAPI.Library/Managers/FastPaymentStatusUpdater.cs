	...
	protected override async Task ExecuteAsync(CancellationToken stoppingToken)
	{
		...
		while(!stoppingToken.IsCancellationRequested)
		{
			...
			try
			{
					...
					using(var scope = _serviceScopeFactory.CreateScope())
					{
						...
						foreach(var payment in processingFastPayments)
						{
							...
							else
							{
								...
								_logger.LogInformation($"Обновляем статус платежа с сессией: {payment.Ticket} новый статус: {newStatus}");
				...
				_logger.LogInformation(count > 0 ? $"{count} платежей поменяли свой статус" : "Не обнаружено обрабатывающихся платежей");
			}
			catch(Exception e)
			{
