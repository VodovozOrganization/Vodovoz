...
namespace BitrixIntegration {
     public class ProductImporter
    {
        ...
        private readonly BitrixNomenclatureFactory bitrixNomenclatureFactory;
        ...
        public ProductImporter(IUnitOfWork uow, BitrixNomenclatureFactory bitrixNomenclatureFactory, Dictionary<string, ProductGroup> productGroups, ProductGroup unknownGroup)
        {
            this.uow = uow ?? throw new ArgumentNullException(nameof(uow));
            this.bitrixNomenclatureFactory = bitrixNomenclatureFactory ?? throw new ArgumentNullException(nameof(bitrixNomenclatureFactory));
        ...
        private void AddOrUpdateNomenclature(Product product)
        {
            ...
            try {
                if (nomenclatures.TryGetValue(product.Id.ToString(), out Nomenclature nomenclature)) {
                ...
                if (nomenclatures.TryGetValue(product.Id.ToString(), out Nomenclature nom)) {
        ...
        {
            if(nomenclature.OnlineStoreExternalId != product.Id.ToString()) {
                return;
            }
        ...
        {
            var nomenclature = bitrixNomenclatureFactory.CreateNewNomenclature(uow);
            nomenclature.OnlineStore = onlineStore;
            FillNomenclatureFromProduct(nomenclature, product);
            nomenclatures.Add(product.Id.ToString(), nomenclature);
            uow.Save(nomenclature);
        }

        private void FillNomenclatureFromProduct(Nomenclature nomenclature, Product product)
        {
            if(!productGroups.TryGetValue(product.Category.ValueId, out ProductGroup productGroup)) {
                productGroup = unknownProductGroup;
            }
            
            nomenclature.Name = product.Name;
            nomenclature.OfficialName = product.Name;
            nomenclature.OnlineStoreExternalId = product.Id.ToString();
            nomenclature.ProductGroup = productGroup;
                
            SetPrice(nomenclature, product.Price.ToString());
        }
