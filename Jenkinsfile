parallel 'Gtk 2.12.45': {
	...
	stage('For Vod7'){
		node() {
			...
			//stage('Build') {
				sh 'msbuild /p:Configuration=DebugWin /p:Platform=x86 Vodovoz/Vodovoz.sln'
				fileOperations([fileDeleteOperation(excludes: '', includes: 'VodovozVod7.zip')])
				zip zipFile: 'VodovozVod7.zip', archive: false, dir: 'Vodovoz/Vodovoz/bin/DebugWin'
				archiveArtifacts artifacts: 'VodovozVod7.zip', onlyIfSuccessful: true
			//}
		}
		node('Vod7') {
			//stage('Deploy'){
				echo "Checking the deployment for a branch " + env.BRANCH_NAME
				script{
					def OUTPUT_PATH = "C:\\WORK\\_BUILDS\\" + env.BRANCH_NAME
					if(
						env.BRANCH_NAME == 'master'
						|| env.BRANCH_NAME == 'develop'
						|| env.BRANCH_NAME == 'Beta'
						|| env.BRANCH_NAME ==~ /^[Rr]elease(.*?)/)
					{
						echo "Deploy branch " + env.BRANCH_NAME
						copyArtifacts(projectName: '${JOB_NAME}', selector: specific( buildNumber: '${BUILD_NUMBER}'));
						unzip zipFile: 'VodovozVod7.zip', dir: OUTPUT_PATH
					}else{
						echo "Nothing to deploy"
					}
				}
