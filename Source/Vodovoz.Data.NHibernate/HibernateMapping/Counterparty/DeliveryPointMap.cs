...
namespace Vodovoz.Data.NHibernate.HibernateMapping.Counterparty
{
	public class DeliveryPointMap : ClassMap<DeliveryPoint>
	{
		public DeliveryPointMap()
		{
			...
			HasMany(x => x.Phones).KeyColumn("delivery_point_id")
				.Cascade.AllDeleteOrphan()
				.Inverse()
				.LazyLoad();

			HasMany(x => x.NomenclatureFixedPrices).KeyColumn("delivery_point_id")
				.Cascade.AllDeleteOrphan()
				.Inverse()
				.LazyLoad();

			HasMany(x => x.ResponsiblePersons).KeyColumn("delivery_point_id")
				.Cascade.AllDeleteOrphan()
				.Inverse()
				.LazyLoad();

			HasMany(x => x.DeliveryPointEstimatedCoordinates).KeyColumn("delivery_point_id")
				.Cascade.All()
				.Inverse()
				.LazyLoad();
		}
