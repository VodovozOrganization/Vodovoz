...
namespace VodovozMangoService.HostedServices
{
	public class NotificationHostedService : NotificationService.NotificationServiceBase, IHostedService
	{
		...
		private void SendIncome(CallInfo info)
		{
			...
			#if DEBUG
			_logger.Debug($"Для звонка на {info.LastEvent.To.ExtensionUint} подходит {subscriptions.Count} из {Subscribers.Count} подписчиков.");
			#endif
...
#if DEBUG
			_logger.Debug($"Для исходящего с {info.LastEvent.From.ExtensionUint} подходит {subscriptions.Count} из {Subscribers.Count} подписчиков.");
#endif
		...
		public async Task StartAsync(CancellationToken cancellationToken)
		{
			...
			_server = new Server
			{
				...
				Ports = { new ServerPort("0.0.0.0", int.Parse(_configuration["MangoService:grps_client_port"]), ServerCredentials.Insecure) }
			};
			_server.Start();
		}
