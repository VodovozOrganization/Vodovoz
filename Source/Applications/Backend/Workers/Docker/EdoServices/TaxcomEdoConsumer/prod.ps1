...
$projectFile = "TaxcomEdoConsumer.csproj"
$baseImageName = "taxcom-docflow.edo-consumer"
$tagName = "latest"
...
Write-Host -ForegroundColor DarkCyan ("2. Push as `"taxcom-docflow-vv.edo-consumer`"")
Write-Host -ForegroundColor DarkCyan ("3. Push as `"taxcom-docflow-vv-north.edo-consumer`"")
Write-Host -ForegroundColor DarkCyan ("4. Push as `"taxcom-docflow-vv-south.edo-consumer`"")
Write-Host -ForegroundColor DarkCyan ("5. Push as `"taxcom-docflow-beverages-world.edo-consumer`"")
Write-Host -ForegroundColor DarkCyan ("6. Push as `"taxcom-docflow-non-alcoholic-beverages-world.edo-consumer`"")
...
for ($i = 1; $i -le $userResponse.Length; $i++) {
    switch ($userResponse[$i-1]) {
        ...
        2 {
            Push -ImageName "taxcom-docflow-vv.edo-consumer"
            break
        }
        3 {
			Push -ImageName "taxcom-docflow-vv-north.edo-consumer"
            break
        }
        4 {
			Push -ImageName "taxcom-docflow-vv-south.edo-consumer"
            break
        }
        5 {
			Push -ImageName "taxcom-docflow-beverages-world.edo-consumer"
            break
        }
        6 {
			Push -ImageName "taxcom-docflow-non-alcoholic-beverages-world.edo-consumer"
            break
        }
        default {
