	...
	public class OnlineOrderFactoryV5 : IOnlineOrderFactoryV5
	{
			...
			IEnumerable<OnlineOrderItemDtoV5> onlineOrderItemsDtos)
		{
			...
			foreach(var onlineOrderItemDto in onlineOrderItemsDtos)
			{
				...
				//TODO 5695: сейчас модель не принимает множество скидок, поэтому передаем первую скидку
				
				var firstDiscount = onlineOrderItemDto.Discounts.FirstOrDefault();
				
				if(firstDiscount is null)
				{
					throw new InvalidOperationException("Не переданы скидки в заказе");
				}
				
				if(firstDiscount.DiscountReasonId.HasValue)
				{
					applicableDiscountReason = uow.GetById<DiscountReason>(firstDiscount.DiscountReasonId.Value);
				}
