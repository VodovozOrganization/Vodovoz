...
{
	/// <summary>
	/// Отправляет смс уведомление о том, что курьер в пути,
	/// клиентам, не пользующимся мобильным приложением
	/// </summary>
	internal class CourierOnTheWaySmsInformerWorker : SmsInformerWorkerBase
	{
		public CourierOnTheWaySmsInformerWorker(
			IOptions<SmsInformerOptions> options,
			ILogger<CourierOnTheWaySmsInformerWorker> logger,
		...
		public override void SendNotification(SmsNotification notification)
		{
			try
			{
				...
				_logger.LogInformation(
					"Отправлено уведомление о том, что курьер в пути. Тел.: {MobilePhoneNumber}, результат: {SendingResult}",
					smsMessage.MobilePhoneNumber,
					result.GetEnumTitle());
		...
		{
			_logger.LogInformation("Остановлена отправка смс уведомлений о том, что курьер в пути.");
			base.OnStopService();
		}

		public override IEnumerable<SmsNotification> GetNotifications(IUnitOfWork unitOfWork, IServiceProvider serviceProvider) =>
			serviceProvider.GetRequiredService<ISmsNotificationRepository>().GetUnsendedCourierOnTheWaySmsNotifications(unitOfWork);
	}
}
