...
namespace Vodovoz.Additions.Logistic.RouteOptimization
{
	...
	public class RouteOptimizer : PropertyChangedBase, IRouteOptimizer
	{
		...
		private ExtDistanceCalculator _distanceCalculator;
		...
		public void CreateRoutes(DateTime date, TimeSpan drvStartTime, TimeSpan drvEndTime)
		{
			...
			_logger.LogInformation("Развозка по {DistrictCount} районам.", calculatedOrders.Select(x => x.District).Distinct().Count());
			...
			if(WarningMessages.Any() && !_interactiveService.Question(
			...
			if(solution != null)
			{
				...
				{
					var route = new ProposedRoute(possibleRoutes[routeNumber], _interactiveService);
