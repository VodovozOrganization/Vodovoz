...
namespace Vodovoz.Filters.GtkViews
{
	...
	public partial class DebtorsFilterView : FilterViewBase<DebtorsJournalFilterViewModel>
	{
		...
		private void Configure()
		{
			...
			yvalidatedentryDebtTo.Binding.AddBinding(ViewModel, x => x.DebtBottlesTo, x => x.Text, new NullableIntToStringConverter()).InitializeFromSource();
			yvalidatedentryDebtFrom.Binding.AddBinding(ViewModel, x => x.DebtBottlesFrom, x => x.Text, new NullableIntToStringConverter()).InitializeFromSource();
			yvalidatedentryBottlesTo.Binding.AddBinding(ViewModel, x => x.LastOrderBottlesTo, x => x.Text, new NullableIntToStringConverter()).InitializeFromSource();
			yvalidatedentryBottlesFrom.Binding.AddBinding(ViewModel, x => x.LastOrderBottlesFrom, x => x.Text, new NullableIntToStringConverter()).InitializeFromSource();
			yvalidatedentryDeliveryPointsTo.Binding.AddBinding(ViewModel, x => x.DeliveryPointsTo, x => x.Text, new NullableIntToStringConverter()).InitializeFromSource();
			yvalidatedentryDeliveryPointsFrom.Binding.AddBinding(ViewModel, x => x.DeliveryPointsFrom, x => x.Text, new NullableIntToStringConverter()).InitializeFromSource();
			...
			nullablecheckOneOrder.Binding.AddBinding(ViewModel, vm => vm.WithOneOrder, w => w.Active).InitializeFromSource();
