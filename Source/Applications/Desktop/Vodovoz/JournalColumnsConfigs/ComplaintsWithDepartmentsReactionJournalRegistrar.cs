		...
		public override IColumnsConfig Configure(FluentColumnsConfig<ComplaintWithDepartmentsReactionJournalNode> config) =>
			config.AddColumn("№ п/п").HeaderAlignment(0.5f)
				...
				.AddColumn("В работе у").HeaderAlignment(0.5f)
					...
					.XAlign(0f)
				.AddColumn("Время\nподключения\nотдела").HeaderAlignment(0.5f)
					.AddTextRenderer(node => node.DepartmentConnectionTimeString)
					.XAlign(0f)
				.AddColumn("Время\nпервого\nкомментария").HeaderAlignment(0.5f)
					.AddTextRenderer(node => node.DepartmentFirstCommentTimeString)
					.XAlign(0f)
				.AddColumn("Время\nреакции").HeaderAlignment(0.5f)
					.AddTextRenderer(node => node.DepartmentReactionTimeString)
					.XAlign(0f)
				...
				.AddSetter<CellRenderer>(
					(cell, node) =>
					{
						...
						if(node.Status != ComplaintStatuses.Closed && node.LastPlannedCompletionDate.Date < DateTime.Today)
						{
