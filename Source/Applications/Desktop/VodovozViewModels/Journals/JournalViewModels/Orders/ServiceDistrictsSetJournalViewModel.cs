...
namespace Vodovoz.Journals.JournalViewModels
{
	public sealed class ServiceDistrictsSetJournalViewModel : EntityJournalViewModelBase<ServiceDistrictsSet, ServiceDistrictsSetViewModel, ServiceDistrictsSetJournalNode>
	{
				  ...
				  currentPermissionService)
		{
			...
			_canActivateServiceDistrictsSet = CurrentPermissionService.ValidatePresetPermission(Permissions.ServiceDistrictsSet.CanActivateServiceDistrictsSet);
			...
			PopupActionsList.Add(
				new JournalAction(
					...
					selectedItems =>
					{
						...
						var activeServiceDistrictsSet = UoW.Session.QueryOver<ServiceDistrictsSet>().Where(x => x.Status == ServiceDistrictsSetStatus.Active).Take(1).SingleOrDefault();
