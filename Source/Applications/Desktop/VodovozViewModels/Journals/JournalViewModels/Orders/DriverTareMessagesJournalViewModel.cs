...
namespace Vodovoz.ViewModels.Journals.JournalViewModels.Orders
{
	public class DriverTareMessagesJournalViewModel : FilterableMultipleEntityJournalViewModelBase<DriverMessageJournalNode, DriverMessageFilterViewModel>
	{
		...
		{
			_autoRefreshTimer = new Timer(600000);
		...
		{
			var colNames = new string[] { "Код", "Дата", "Время", "ФИО водителя", "Телефон водителя", "№ МЛ", "№ заказа", "План бут.", "Факт бут.", "Долг бут. по адресу", "Комментарий водителя", "Комментарий ОП/ОСК", "Автор комментария", "Время комментария", "Время реакции" };
			...
			var rows = from row in a
					   select new
					   {
						   ...
						   CommentDate = row.CommentDate.ToString("dd.MM.yy"),
						   row.DriverName,
						   row.DriverPhone,
						   row.RouteListId,
						   row.OrderId,
						   ...
						   row.DriverComment,
						   row.CommentManager,
						   lastEditor = row.LastCommentManagerEditor.FullName,
						   lastEditTIme = row.LastCommentManagerEditedTime.ToString("HH:mm:ss dd.MM.yy"),
						   row.ResponseTime
					   };
			int index = 1;
