	...
	public class ExportData : IXmlConvertable
	{
		...
		public TableRecordNode CreateRecord(OrderItem orderItem)
		{
			...
			if(ExportMode == Export1cMode.ComplexAutomation)
			{
				...
				{
					Vat = orderItem.Nomenclature.GetActualVatRateVersion(orderItem.Order.DeliveryDate)?.VatRate
				};
			...
			{
				var vat = orderItem.Nomenclature.GetActualVatRateVersion(orderItem.Order.DeliveryDate)?.VatRate.GetValue1c();
			...
			if(orderItem.Nomenclature.GetActualVatRateVersion(orderItem.Order.DeliveryDate)?.VatRate.VatRateValue != 0)
			{
				record.Properties.Add(
					new PropertyNode(
						"СуммаНДС",
