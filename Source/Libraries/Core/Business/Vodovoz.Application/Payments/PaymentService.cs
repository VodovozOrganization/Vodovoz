...
namespace Vodovoz.Application.Payments
{
	...
	internal sealed class PaymentService : IPaymentService
	{
			...
			bool distributeCompletedPayments = false)
		{
			...
			if(!orderNodes.Any())
			{
				return Vodovoz.Errors.Payments.PaymentsDistribution.NoOrdersToDistribute(counterpartyId);
			}

			if(!paymentNodes.Any())
			{
				return Vodovoz.Errors.Payments.PaymentsDistribution.NoPaymentsWithPositiveBalance(counterpartyId);
			}
			...
			foreach(var paymentNode in paymentNodes)
			{
				...
				{
					return Vodovoz.Errors.Payments.PaymentsDistribution.AutomaticDistribution(e.Message);
				}
			}
