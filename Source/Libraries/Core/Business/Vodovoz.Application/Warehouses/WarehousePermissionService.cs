...
namespace Vodovoz.Application.Warehouses
{
	...
	/// 
	/// Предполагаю, что алгоритм должен быть заменен:
	/// - Правыа должны применяться сверху вниз
	///  - Права корневого подразделения
	///  - Права низлежащих подразделений, перезаписывающие текущие разрешения
	///  - Права пользователя, перезаписывающие текущие разрешения
	/// 
	/// Такой подход позволяет гибко настраивать доступ, например: разрешить для всех, но запретить подразделению, но дать доступ 1му пользователю из этого подразделения
	/// </summary>
