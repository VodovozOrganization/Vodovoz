...
{
	public class SalesDetailReportModifier : ReportModifierBase
	{
		private const string _tableName = "TableSales";
		private const string _firstItemDataTextboxName = "TextboxFirstItemData";
		private const string _phonesTextBox = "TextboxPhone";
		private const uint _firstItemDataColumnSpan = 6;
		private bool _hidePhones = false;
		...
		public SalesDetailReportModifier()
		{
			_expressionRowProvider = new FooterExpressionRowProvider();
			_sourceRowProvider = new DetailsSourceRowProvider();
		}

		public void Setup(IEnumerable<GroupingType> groupings, bool hidePhones)
		{
			_hidePhones = hidePhones;

			if(hidePhones)
			{
				var removePhonesColumn = new RemoveColumn(_tableName, _phonesTextBox);
				AddAction(removePhonesColumn);
			}
		...
		private SetTableCellColumnSpan GetFirstItemColSpanUpdateAction(string groupName)
		{
			...
			uint span = _hidePhones ? _firstItemDataColumnSpan - 1 : _firstItemDataColumnSpan;

			var action = new SetTableCellColumnSpan(_tableName, textBoxName, span);
			return action;
		}
