...
namespace VodovozBusiness.Domain.Sale.RequestsForCall
{
	...
	[EntityPermission]
	[HistoryTrace]
	public abstract class RequestForCallBase : PropertyChangedBase, IDomainObject, IValidatableObject
	{
		...
		/// Тип заявки
		/// </summary>
		public abstract RequestForCallType Type { get; }
		
		/// <summary>
		/// Сервисная заявка
		/// </summary>
		public virtual bool IsService => Type == RequestForCallType.Service;
		...
		protected static T CreateNew<T>(
			Source source,
			string contactName,
			string phoneNumber,
			Nomenclature nomenclature,
			Counterparty counterparty)
			where T : RequestForCallBase, new()
		{
			var requestForCall = new T
			{
				Source = source,
				Author = contactName,
				Phone = phoneNumber,
