...
namespace Vodovoz.Core.Data.NHibernate.Operations
{
	public class CounterpartyMovementOperationMap : ClassMap<CounterpartyMovementOperation>
	{
		public CounterpartyMovementOperationMap()
		{
			...
			References(x => x.WriteoffCounterparty)
				.Column("writeoff_counterparty_id");

			References(x => x.IncomingCounterparty)
				.Column("incoming_counterparty_id");

			References(x => x.WriteoffDeliveryPoint)
				.Column("writeoff_delivery_point_id");

			References(x => x.IncomingDeliveryPoint)
				.Column("incoming_delivery_point_id");
		}
	}
}
