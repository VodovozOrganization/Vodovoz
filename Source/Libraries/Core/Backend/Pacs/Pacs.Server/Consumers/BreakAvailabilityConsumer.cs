...
{
	/*public class BreakAvailabilityConsumer : IConsumer<OperatorBreakAvailability>
	{
		private readonly IOperatorControllerProvider _operatorControllerProvider;

		public BreakAvailabilityConsumer(IOperatorControllerProvider operatorControllerProvider)
		{
			_operatorControllerProvider = operatorControllerProvider ?? throw new ArgumentNullException(nameof(operatorControllerProvider));
		}

		public async Task Consume(ConsumeContext<OperatorBreakAvailability> context)
		{
			var controller =  _operatorControllerProvider.GetOperatorController(context.Message.OperatorId);
