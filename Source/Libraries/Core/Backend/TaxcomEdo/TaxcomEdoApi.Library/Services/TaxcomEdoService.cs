		...
		public TaxcomContainer CreateContainerWithUpd(UniversalTransferDocumentInfo updInfo)
		{
			...
			if(!upd.Validate(out var errors))
			{
				...
				_logger.LogError(
					...
					updInfo.Date >= new DateTime(2026,1,1) ? updInfo.StringNumber : updInfo.Number,
					updInfo.DocumentId,
					errorsString);

				throw new InvalidOperationException($"УПД {(updInfo.Date >= new DateTime(2026,1,1) ? updInfo.StringNumber : updInfo.Number)} {updInfo.DocumentId} не прошла валидацию, отправка не возможна");
				//подумать, что делаем в таких случаях
			}
