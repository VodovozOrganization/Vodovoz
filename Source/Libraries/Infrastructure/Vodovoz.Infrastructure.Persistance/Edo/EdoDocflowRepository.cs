...
";
		...
		public IEnumerable<SaveCodesEdoTask> GetClientSavedToPoolDocumentTaskIdsForResend(
			IUnitOfWork uow,
			int counterpartyId,
			IEnumerable<OrderStatus> excludedOrderStatuses)
		{
			...
			return tasks
				.Where(task =>
					...
					&& !excludedOrderStatuses.Contains(task.FormalEdoRequest.Order.OrderStatus)
					&& !tasks.Any(otherTask =>
