...
// 					var alreadyCopiedDistrict = UoW.Session.QueryOver<Sector>().WhereRestrictionOn(x => x.CopyOf.Id).IsIn(districtsSetToCopy.Districts.Select(x => x.Id).ToArray()).Take(1).SingleOrDefault();
...
// 						&& selectedItems.OfType<DistrictsSetJournalNode>().FirstOrDefault()?.Status == SectorsSetStatus.Draft,
// 					selectedItems => true,
// 					selectedItems => {
...
// 						if(selectedNode == null)
// 							return;
// 						var activeDistrictsSet = UoW.Session.QueryOver<DistrictsSet>().Where(x => x.Status == SectorsSetStatus.Active).Take(1).SingleOrDefault();
...
// 					"В черновик",
// 					selectedItems => canUpdate &&
// 						selectedItems.OfType<DistrictsSetJournalNode>().FirstOrDefault()?.Status == SectorsSetStatus.Closed,
// 					selectedItems => true,
// 					selectedItems => {
// 						var selectedNodes = selectedItems.OfType<DistrictsSetJournalNode>();
// 						var selectedNode = selectedNodes.FirstOrDefault();
